...
export class SuccessfulProjectItem extends React.Component {
    ...
    getData = () => {
        axios.get(`http://localhost:8091/api/v1/project/${this.props.project.id}/getAvatar`)
            .then(response => this.setState({ avatar: response.data }));
    };
            ...
            <div>
                <Link to={`projects/${this.props.project.id}`} style={{ textDecoration: 'none', color: 'black' }}>
                    <Card
                        ...
                        style={{
                            height: '18rem', width: '23rem', marginTop: '1rem', marginBottom: '2.5rem',
                        }}
                    >
                        <Card.Body>
                            <Card.Img
                            ...
                            <ProgressBar
                                doneTip="Зібрано!"
                                donePercent={100}
                                doneLabel={this.props.project.moneyNeeded}
